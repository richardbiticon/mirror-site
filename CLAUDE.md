...
1. `/docs/02-brand-guide.md`: visual and verbal constitution. Wins ties.
...
- **Slow animations only.** 600 to 800ms for entrances, 300 to 400ms for hovers. Easing is locked to `cubic-bezier(0.16, 1, 0.3, 1)` (CSS var `--ease-mirror`).
...
After every section ships, screenshot desktop and mobile and verify **none** of these are present. If any appear, the section is wrong and gets rebuilt:
...
- Radix primitives (accordion, dialog) wrapped in branded components in `/components/ui/`. The brand guide calls these "shadcn primitives"; we install Radix directly because we override every default style anyway, so importing the shadcn cosmetic shell would just create dead classes to delete.
...
Do not add framer-motion separately. Do not add a UI kit beyond the Radix primitives listed. Do not add Storybook. Do not add a CMS.
...
- Particle counts within stated ranges (80 to 120 desktop, 40 mobile)
